...
__m256i wang_hash_ps(__m256i& seeds)
{
    ...
    seeds = _mm256_xor_si256(seeds, _mm256_srai_epi32(seeds, 4));
    seeds = _mm256_mullo_epi32(seeds, _mm256_set1_epi32(0x27d4eb2d));
    seeds = _mm256_xor_si256(seeds, _mm256_srai_epi32(seeds, 15));
    return seeds;
}
...
static m256x3 mainImage(m256x2 fragCoord, m256x2 iResolution, f32 iFrame)
{
   ...
   /* __m256i rngState_epi =
    ...
    for (int index = 0; index < c_numRendersPerFrame; ++index)
        color += (GetColorForRay(rayPosition, rayDir, rngState) * (1.f / (c_numRendersPerFrame)));
